...
const FinalFormTest = ({ fieldCount = 0, onSubmit }) => {
  ...
  const keys = [...Array(fieldCount).keys()].map(n => n + 1)
          ...
          return (
            <Row>
              <Col style={{ paddingRight: '1rem' }}>
                ...
                <p>
                  Click <strong>Reset</strong> and input something for the first 10 text fields (marked as required).
                  Your goal is to minimze the form render count without exceeding 2 renders for a successful form
                  submission. So each time you make change to a text field, it will not rerender the whole form on every
                  change.
                </p>
                <FormGroup>
                ...
                <FormGroup>
                  ...
                  <FormSpy subscription={{ values: true }}>
                    {({ values }) => <Preview>{JSON.stringify(values, 0, 2)}</Preview>}
                  </FormSpy>
                </FormGroup>
              </Col>
                  ...
                  {({ FormContainer, FormRow, FormCol }) => (
                    <FormContainer style={{ width: '100%' }}>
                      {keys.map((key, index) => {
                        ...
                        return (
                          <Field name={`field${key}`} key={key} validate={validate}>
                            {({ input, meta }) => (
                              <FormRow>
                                <FormCol style={{ width: '1%', whiteSpace: 'nowrap' }}>
                                  <Label required={isRequiredField}>Field {key}</Label>
                                </FormCol>
                                <FormCol>
